...
public class ICCategoryBMPBean extends com.idega.data.TreeableEntityBMPBean implements ICCategory, Category, MetaDataCapable {
	...
	/**
	 * basically returns the same as ejbHomeGetListOfCategoryForObjectInstance() but does include only root categories in the list
	 * 
	 * @param obj
	 * @param order
	 * @return
	...
	public List ejbHomeGetListOfRootCategoryForObjectInstance(ICObjectInstance obj, boolean order) throws FinderException {
		 ...
		 * 
		 * LEFT JOIN IC_CATEGORY_IC_OBJECT_INSTANCE mt ON cat.IC_CATEGORY_ID= mt.IC_CATEGORY_ID where tree.child_ic_category_id is null and
		 * mt.IC_OBJECT_INSTANCE_ID = 9
		 */
	...
	public int ejbHomeGetOrderNumber(Category category, String objectInstanceId) throws javax.ejb.FinderException {
		try {
			String query = "SELECT TREE_ORDER FROM " + EntityControl.getManyToManyRelationShipTableName(ICCategory.class, ICObjectInstance.class) + " WHERE " + IC_OBJECT_INSTANCE_COLUMN_NAME + " = " + objectInstanceId + " AND " + this.IC_CATEGORY_COLUMN_NAME + " = " + category.getID();
	...
	public boolean ejbHomeSetOrderNumber(Category category, ICObjectInstance instance, int orderNumber) throws com.idega.data.IDOException {
		...
		// if (orderNumber == 0) {
		// return this.idoExecuteTableUpdate("UPDATE "+EntityControl.getManyToManyRelationShipTableName(ICCategory.class, ICObjectInstance.class)+" SET
		// "+TREE_ORDER_COLUMN_NAME+" AS null WHERE "+IC_OBJECT_INSTANCE_COLUMN_NAME+" = "+instance.getID()+" AND "+IC_CATEGORY_COLUMN_NAME+" =
		// "+category.getID());
		// }else {
	...
	/**
	 * faster method than the nested one, and works on mysql SELECT cat.* FROM ic_category cat LEFT JOIN ic_category_tree tree ON
	 * cat.ic_category_id=tree.child_ic_category_id where cat.cat_type = 'news' and tree.child_ic_category_id is null
	 * 
	 * @param type
	 * @return
	 */
	 ...
	 */
	/*
	 * private String getRootsNestedSQL(String type) { StringBuffer sql = new StringBuffer("select * from "); sql.append(getEntityTableName());
	 * 
	 * sql.append(" where ").append(getColumnType()).append("= '").append(type).append("' "); sql.append(" and ").append(getIDColumnName()).append(" not
	 * in ("); sql.append(" select ").append("child_").append(getIDColumnName()); sql.append(" from ").append(getEntityTableName()).append("_tree )");
	 * return sql.toString(); }
	 */
